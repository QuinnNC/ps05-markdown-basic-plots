...
gapMinder %>% 
  ...
  filter(time <= 2016) %>% 
  summarise(aveEmission = mean(co2_PC)) %>% 
  ggplot(aes( time, aveEmission, fill = region)) +
  geom_col()+
  labs(title = "Average CO2 emissions per capita over time across the Continents (1960 - 2016)",
       x = "time (year)",
...
- Took about 1hr 30 min to set up the assignment. (10:30am - 12pm)
- Left and came back to this assignment (2pm - 5pm), next day (7pm - 8:30pm)()
